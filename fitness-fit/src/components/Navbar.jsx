    ...
    <header className={`sticky top-0 z-50 transition ${scrolled ? 'backdrop-blur bg-white/70 dark:bg-gray-900/60 shadow-sm' : ''}`}>
      <nav className="container-section flex items-center justify-between py-3">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <ScrollLink key={item.to} to={item.to} smooth spy offset={-80} className="cursor-pointer text-sm font-medium text-gray-700 hover:text-red-600 dark:text-gray-300">
              {item.label}
            </ScrollLink>
          ))}
        </div>
      ...
      {isOpen && (
        <div className="md:hidden border-t border-gray-200 dark:border-gray-800">
          <div className="container-section py-3 flex flex-col gap-3">
            {navItems.map((item) => (
              <ScrollLink key={item.to} to={item.to} smooth spy offset={-80} className="cursor-pointer py-2 text-sm font-medium text-gray-700 hover:text-indigo-600 dark:text-gray-300" onClick={() => setIsOpen(false)}>
                {item.label}
              </ScrollLink>
            ))}
          </div>
